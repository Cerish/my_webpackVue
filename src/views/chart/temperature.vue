...
export default {
    ...
    methods: {
        ...
        async setDateArr(num) {
            let time = new Date().getTime();
            let distanceTime = 24 * 60 * 60 * 1000;
            for(let i = 0; i < num; i++) {
                let getDate = await this.getDate(time + distanceTime * i);
                this.dateArr.push(getDate);
            }
            this.options.xAxis[0].data = this.dateArr;
        }
    },
    created() {
        // 参数为设置天气展示的天数
        this.setDateArr(7);
        // https://gallery.echartsjs.com/editor.html?c=xl4d5upNIN
